...
function calculateAverage(m: Array<number>): number {
  const avg: number = m.reduce((a, b) => a + b, 0) / m.length;
  return avg;
}

function updateStatus(s: Student): void {
...
function assignGrade(s: Student): void {
  const avg = s.average!;
  if (avg >= 90) s.grade = "A";
  else if (avg >= 80) s.grade = "B";
  else if (avg >= 70) s.grade = "C";
  else if (avg >= 60) s.grade = "D";
  else if (avg >= 50) s.grade = "E";
  else s.grade = "F";
}

function passStudents(students: Student[]): string[] {
  return students
    .filter((stud) => stud.status === "pass")
    .map((stud) => stud.name);
}

function failStudents(students: Student[]): string[] {
  return students
...
students.forEach((s) => {
  console.log(
    `Student ${s.name} got ${s.grade} grade & has ${s.status} with average of ${s.average} marks.`
  );
});
